...
from flaskblog import app, db, bcrypt, authorize, mail
from flaskblog.forms import RegistrationForm, LoginForm, UpdateAccountForm, \
    Report, attendancesForm, LeaveForm, SubjectForm, TermForm, RequestResetForm, ResetPasswordForm
...
def notices():
    ...
    notices = Notice.query.order_by(asc(Notice.data_posted)).all()
...
def leaves(leave_id):
    ...
    return render_template('leave.html', reason=leave.reason, leave=leave,
                           form=form)
...
def attendances():
    student_attendances = Attendance.query.order_by(
        asc(Attendance.data_posted)).all()
