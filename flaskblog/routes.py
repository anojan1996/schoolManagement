...
from flaskblog.forms import RegistrationForm, LoginForm, UpdateAccountForm, studentsForm, attendancesForm, leavesForm, noticesForm
from flaskblog.models import User, students, attendances, leaves, notices
...
def notice():
    form = noticesForm()
    if form.validate_on_submit():
        notice = notices(title=form.title.data,
                         content=form.content.data)
        db.session.add(notice)
        db.session.commit()
        flash('Notice Added Successfully')
        return redirect(url_for('show_all'))
    return render_template('notice.html', form=form)
...
def view():
    ...
    return render_template('view.html', image_file=image_file, title='View', student=student)
